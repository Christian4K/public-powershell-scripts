<#
.SYNOPSIS
    Remove all objects from the database to prepare for a clean import

.DESCRIPTION
    This script will remove all objects from the database to prepare for a clean import. 
    It will drop all views, procedures, functions, constraints, and tables from the database.
    It will add the AzureSQLMaintenance procedure to the database.

.PARAMETER ParameterName
    DatabaseServer: The server name of the database
    Database: An array of strings to filter the databases to clean, e.g "qhse", "surveys"
    debug: A boolean to enable debug mode

.EXAMPLE
    # Sample to check databases sharedfree on server cv001.database.windows.net
    .\Clean-Database.ps1 -DatabaseServer cv001.database.windows.net -Database "sharedfree" -debug $true
#>

param (
	[string]    $DatabaseServer = "myserver.database.windows.net",
	[string]    $Database = "cleandb",
	[bool]      $debug = $false,
	[bool]      $AzureSQLMaintenance = $false
)
...
function CleanDatabase {
	...
	Write-Host "------------------------------------------------------------------------"    
	Write-Host "Cleaning database name $dbName"

	Write-Host "Dropping views"
	$sql = [SqlGenerator]::GetDropViewsSQL() 
	Invoke-SqlCmd -ServerInstance "$DatabaseServer" `
		-Database "$dbName" `
		-AccessToken "$token" `
		-Query $sql

	Write-Host "Dropping procedures"
	$sql = [SqlGenerator]::GetDropProcsSQL() 
	Invoke-SqlCmd -ServerInstance "$DatabaseServer" `
		-Database "$dbName" `
		-AccessToken "$token" `
		-Query $sql

	Write-Host "Dropping functions"
	$sql = [SqlGenerator]::GetFunctionsSQL()
	Invoke-SqlCmd -ServerInstance "$DatabaseServer" `
		-Database "$dbName" `
		-AccessToken "$token" `
		-Query $sql

	Write-Host "Dropping Constraints"
	$sql = [SqlGenerator]::GetDropConstraintsSQL() 
	Invoke-SqlCmd -ServerInstance "$DatabaseServer" `
		-Database "$dbName" `
		-AccessToken "$token" `
		-Query $sql
	...
	$sql = [SqlGenerator]::GetDropTablesSQL()
	Invoke-SqlCmd -ServerInstance "$DatabaseServer" `
		-Database "$dbName" `
		-AccessToken "$token" `
		-Query $sql

	if ($AzureSQLMaintenance) {
		Write-Host "Add AzureSQLMaintenance"
		$sql = [SqlGenerator]::GetProcdureAzureSQLMaintenance()
		Invoke-SqlCmd -ServerInstance "$DatabaseServer" `
			-Database "$dbName" `
			-AccessToken "$token" `
			-Query $sql
...
Write-Host "Cleaning databases on server $DatabaseServer"
...
# Get the list of databases for the current tenant
try {
	$sql = "SELECT name FROM sys.databases WHERE name = '$Database' AND name NOT IN ('master', 'tempdb', 'model', 'msdb')"

	$databases = Invoke-SqlCmd -ServerInstance "$DatabaseServer" `
		-Database "master" `
		-AccessToken "$token" `
		-Query $sql
	...
	Write-Host "Failed to get the list of databases for name $Database in the server $DatabaseServer" -f Red
	Write-Host $_.Exception -f Red
	exit 1
}

if ($databases.Count -eq 0) {
	Write-Host "No databases found for name $Database in the server $DatabaseServer"
	exit 1
}


Write-Debug "Checking database $Database"
CleanDatabase -dbName $Database


class SqlGenerator { 
	static [string] GetDropViewsSQL() {
		return @"
Declare @viewName varchar(500) 
Declare cur Cursor For Select [name] From sys.objects where type = 'v' and name <> 'database_firewall_rules' and name <> 'ipv6_database_firewall_rules'
Open cur 
...
"@
	...
	static [string] GetFunctionsSQL() {
		return @"
Declare @functionName varchar(500) 
Declare cur Cursor For Select [name] From sys.objects where type = 'FN' 
Open cur 
...
"@
	...
	static [string] GetDropConstraintsSQL() { 
		return @"
...
"@
	...
	static [string] GetDropTablesSQL() { 
		return @"
...
"@  
	...
	static [string] GetProcdureAzureSQLMaintenance() {
		return @"
